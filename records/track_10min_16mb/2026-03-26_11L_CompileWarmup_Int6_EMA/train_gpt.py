...
class TernaryLinear(nn.Linear):
    """BitNet-style ternary {-1, 0, +1} quantization with STE."""
    _qat_enabled: bool = False

    @staticmethod
    def ternary_quantize(w: Tensor) -> tuple[Tensor, Tensor]:
        """Quantize to {-1, 0, +1} using BitNet b1.58 absmean method."""
        w32 = w.float()
        gamma = w32.abs().mean().clamp_min(1e-7)
        w_ternary = torch.clamp(torch.round(w32 / gamma), -1, 1)
        return w_ternary, gamma

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if TernaryLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                w_ternary, gamma = self.ternary_quantize(self.weight)
                w_q = (w_ternary * gamma).to(x.dtype)
            w = w + (w_q - w).detach()  # STE
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)

# Global flag for ternary mode
_USE_TERNARY: bool = False
_TERNARY_QAT_ENABLED: bool = False

def ternary_ste(w: Tensor) -> Tensor:
    """Apply ternary quantization with STE to a weight tensor.
    Used for bank weights during forward pass when ternary QAT is enabled."""
    if not _TERNARY_QAT_ENABLED:
        return w
    w32 = w.float()
    gamma = w32.abs().mean().clamp_min(1e-7)
    w_ternary = torch.clamp(torch.round(w32 / gamma), -1, 1)
    w_q = (w_ternary * gamma).to(w.dtype)
    return w + (w_q - w).detach()  # STE: forward uses quantized, backward uses original

def ternary_quantize_for_export(t: Tensor) -> tuple[Tensor, Tensor]:
    """Pack ternary weights: 5 values per byte (1.6 bits/weight).
    Returns (packed_uint8, scale_fp16)."""
    t32 = t.float()
    gamma = t32.abs().mean().clamp_min(1e-7)
    w_norm = t32 / gamma
    # -1->0, 0->1, +1->2
    ternary = torch.ones_like(t32, dtype=torch.uint8)
    ternary[w_norm > 0.5] = 2
    ternary[w_norm < -0.5] = 0

    rows, cols = ternary.shape
    pad = (5 - cols % 5) % 5
    if pad > 0:
        ternary = F.pad(ternary, (0, pad), value=1)
    ternary = ternary.reshape(rows, -1, 5)

    # Pack: v0 + 3*v1 + 9*v2 + 27*v3 + 81*v4
    powers = torch.tensor([1, 3, 9, 27, 81], dtype=torch.uint8, device=t.device)
    packed = (ternary * powers).sum(dim=-1).to(torch.uint8)

    return packed.contiguous(), gamma.to(torch.float16)

def ternary_dequantize(packed: Tensor, gamma: Tensor, orig_cols: int) -> Tensor:
    """Unpack ternary weights for inference."""
    rows, packed_cols = packed.shape
    unpacked = torch.zeros(rows, packed_cols, 5, dtype=torch.float32, device=packed.device)
    values = packed.clone().to(torch.int32)
    for i in range(5):
        unpacked[:, :, i] = (values % 3).float() - 1.0
        values = values // 3
    return (unpacked.reshape(rows, -1)[:, :orig_cols] * gamma.float())

def make_linear(in_features: int, out_features: int, bias: bool = False) -> nn.Linear:
    """Factory for Linear layers: TernaryLinear when enabled, else CastedLinear."""
    if _USE_TERNARY:
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # Apply ternary STE to banks if enabled (no-op when _TERNARY_QAT_ENABLED=False)
        qo = ternary_ste(self.qo_bank) if self.training else self.qo_bank
        kv = ternary_ste(self.kv_bank) if self.training else self.kv_bank
        mlp_up = ternary_ste(self.mlp_up_bank) if self.training else self.mlp_up_bank
        mlp_down = ternary_ste(self.mlp_down_bank) if self.training else self.mlp_down_bank
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # Use ternary quantized weights for inference if enabled
        qo = ternary_ste(self.qo_bank) if _TERNARY_QAT_ENABLED else self.qo_bank
        kv = ternary_ste(self.kv_bank) if _TERNARY_QAT_ENABLED else self.kv_bank
        mlp_up = ternary_ste(self.mlp_up_bank) if _TERNARY_QAT_ENABLED else self.mlp_up_bank
        mlp_down = ternary_ste(self.mlp_down_bank) if _TERNARY_QAT_ENABLED else self.mlp_down_bank
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                qo[i], kv[i], kv[n + i],
...
def mixed_quantize_ternary(state_dict: dict[str, Tensor], ternary_cats: set[str]):
    """Quantize state dict with ternary for large matrices, fp16/int8 for others."""
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            meta[name] = "passthrough_ctrl"
            continue
        if cat in ternary_cats and t.ndim == 2:
            packed, gamma = ternary_quantize_for_export(t)
            result[name + ".q"] = packed
            result[name + ".gamma"] = gamma
            meta[name] = {"type": "ternary", "orig_cols": t.shape[1]}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_ternary(result: dict[str, Tensor], meta: dict[str, object],
                              template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Dequantize state dict with ternary/int8 mixed quantization."""
    out: dict[str, Tensor] = {}
    for name, orig in template_sd.items():
        info = meta.get(name)
        if info is None:
        ...
        if isinstance(info, dict) and info.get("type") == "ternary":
            packed = result[name + ".q"]
            gamma = result[name + ".gamma"]
            orig_cols = info["orig_cols"]
            out[name] = ternary_dequantize(packed, gamma, orig_cols).to(orig_dtype)
            continue
        # int8 fallback
        q, s = result[name + ".q"], result[name + ".scale"]
        if s.ndim > 0:
...
def main() -> None:
    ...
    # Setup quantization mode
    global _USE_TERNARY, _TERNARY_QAT_ENABLED
    _USE_TERNARY = args.ternary_enabled
    # QAT flags start disabled - late_qat enables them when LR decays
    CastedLinear._qat_enabled = args.qat_enabled
    TernaryLinear._qat_enabled = False  # enabled by late_qat
    _TERNARY_QAT_ENABLED = False  # enabled by late_qat for bank weights
    # Disable EMA when ternary is enabled (incompatible)
    if args.ternary_enabled and args.ema_enabled:
        log0("WARNING: EMA is incompatible with ternary, disabling EMA")
        args.ema_enabled = False
    if args.ternary_enabled:
        log0(f"quantization:ternary (BitNet 1.58-bit) late_qat_threshold:{args.late_qat_threshold}")
    else:
        log0(f"quantization:int6 (GPTQ-lite)")
    base_model = GPT(
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            if args.ternary_enabled:
                _TERNARY_QAT_ENABLED = True
                TernaryLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} ternary:{args.ternary_enabled}")
        zero_grad_all()
    ...
    # Choose quantization method based on ternary mode
    if args.ternary_enabled:
        quant_result, quant_meta = mixed_quantize_ternary(unbanked_sd, {"mlp", "attn"})
        quant_ext = "ternary"
    else:
    ...
    quant_filename = f"final_model.{quant_ext}.ptz"
    if master_process:
        with open(quant_filename, "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model {quant_ext}+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size {quant_ext}+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    if args.ternary_enabled:
        deq_unbanked = dequantize_mixed_ternary(quant_state["w"], quant_state["m"], unbanked_sd)
    else:
